...
# flex_attention cannot compile with symbolic shapes here, so force static
# compiles: one per distinct sequence length, hence the raised cache limit.
...
class SDARModel(SDARPreTrainedModel):
    def __init__(self, config: SDARConfig):
        ...
        # =====  ===== RELAY: belt-and-braces. _init_weights already zeros this;
        # this guards the case where HF's _fast_init skips the module.=====
        if self.use_relay:
    ...
    def forward(
        ...
        prev_latent: Optional[torch.FloatTensor] = None,        # prev hidden state...
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # ===== RELAY: add LayerNorm(prev_latent) at MASK positions only =====
        if self.use_relay and prev_latent is not None:
...
class SDARForCausalLM(SDARPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        bd_inputs: Optional[Tuple] = None,              # for rollout training...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if self.training:
            ...
            if bd_inputs is None:
                # Pass 1: preserve the original normalization.
                answer_len = (labels != -100).sum()
            else:
                # Pass 2: average over the selected valid targets.
                answer_len = shifted_labels[logits_to_keep_half].ne(-100).sum()
