# Qwen3-8B-b7-causal-from-ar-b3amc (block_size=7, weights from b3-causal checkpoint-32000)
...
# flex_attention cannot compile with symbolic shapes here (fails once torch marks
# the sequence dim dynamic - version_2's "crash at step 8"), so force static
# compiles. That means one compile per distinct sequence length, hence the raised
# cache limit: the default of 8 is exhausted almost immediately with bs=1 data.
...
class SDARPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            # Loopholing bypass gate. from_pretrained builds on the meta device and
            # re-initialises any parameter absent from the checkpoint through this
            # method. With no nn.LayerNorm branch the weight stays raw torch.empty
            # garbage (NaN), the zeros_() in __init__ runs against a meta tensor and
            # does nothing, and 0 * NaN = NaN silently poisons task_loss while
            # clean_ce_loss stays healthy.
...
class SDARModel(SDARPreTrainedModel):
    def __init__(self, config: SDARConfig):
        ...
        # --- Speculative Relay: hidden-state bypass (mechanism 3) ---
        ...
        # Zero the gate so the bypass is an exact no-op at step 0 — that is what
        # makes the no-relay control a valid comparison. AFTER post_init(),
        # because post_init runs _init_weights and would overwrite it.
        if self.use_relay:
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        prev_latent: Optional[torch.FloatTensor] = None,
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.use_relay:
            if prev_latent is None:
                prev_latent = torch.zeros_like(inputs_embeds)
            delta = self.layer_norm(prev_latent).to(inputs_embeds.dtype)
            guard = (input_ids == self.config.mask_token_id).unsqueeze(-1)
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if self.training:
            ...
            if bd_inputs is None:
                ...
                concat_inputs_ids, concat_position_ids, flex_attention_mask_3d, logits_to_keep_half, logits_to_keep, p_mask = bd_inputs
            #concat_inputs_ids, concat_position_ids, flex_attention_mask_3d, logits_to_keep_half, logits_to_keep, p_mask = self.prepare_for_bd_training(input_ids, position_ids, prompt_mask)
            outputs = self.model(
