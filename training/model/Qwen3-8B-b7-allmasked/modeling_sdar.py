...
def anchor_diff_mask(q_idx, kv_idx, block_id, anchor, x0_flag, position, use_regular_causal=True):
    """Slot-indexed mask using per-slot block / anchor / x0 / position tensors.

    Step-1 uniform tiling is the special case ``anchor[noisy] = s_b``.
    """
...
def materialize_anchor_mask(block_id, anchor, x0_flag, position, use_regular_causal=True):
    ...
    return anchor_diff_mask(q_idx, kv_idx, block_id, anchor, x0_flag, position, use_regular_causal)


def build_attention_from_metadata(block_id, anchor, x0_flag, position, use_regular_causal=True):
    """``(B, 2L)`` metadata → flex BlockMask on CUDA, dense bool mask on CPU."""
    if block_id.ndim == 1:
        block_id = block_id.unsqueeze(0)
        anchor = anchor.unsqueeze(0)
        x0_flag = x0_flag.unsqueeze(0)
        position = position.unsqueeze(0)
    batch, total = block_id.shape
    device = block_id.device
    if device.type == "cuda" and is_torch_flex_attn_available():
        def mask_mod(b, h, q_idx, kv_idx):
            return anchor_diff_mask(
                q_idx, kv_idx, block_id[b], anchor[b], x0_flag[b], position[b], use_regular_causal
            )
        return create_block_mask(mask_mod, B=batch, H=None, Q_LEN=total, KV_LEN=total)
    masks = [
        materialize_anchor_mask(block_id[i], anchor[i], x0_flag[i], position[i], use_regular_causal)
...
class SDARAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.training:
            # Dense bool mask (CPU / tests) uses SDPA; flex BlockMask stays on GPU.
...
class SDARForCausalLM(SDARPreTrainedModel, GenerationMixin):
    ...
    def prepare_for_bd_training(self, inputs_ids, position_ids, prompt_mask):
        ...
        # create flex_attention mask (dense bool on CPU so tests / login nodes work)
        ...
        if inputs_ids.device.type == "cuda" and is_torch_flex_attn_available():
            ...
            flex_attention_mask_3d = attention_mask

        return concat_inputs_ids, concat_position_ids, flex_attention_mask_3d, logits_to_keep_half, logits_to_keep, p_mask

    def _inject_relay(self, inputs_embeds, relay_h, relay_mask):
        """Add LN(h) to noisy-half embeddings at ``relay_mask`` slots. Never touches the clean half."""
        if relay_h is None or relay_mask is None or not relay_mask.any():
            return inputs_embeds
        seq_len = relay_mask.shape[-1]
        ln_dtype = self.relay_layer_norm.weight.dtype
        delta = self.relay_layer_norm(relay_h.to(ln_dtype))
        noisy = inputs_embeds[:, :seq_len]
        guard = relay_mask.unsqueeze(-1).to(dtype=torch.bool)
        noisy = torch.where(guard, noisy + delta.to(noisy.dtype), noisy)
        return torch.cat([noisy, inputs_embeds[:, seq_len:]], dim=1)

    def _masked_diffusion_loss(self, hidden_kept, target, p_mask, answer_len):
        use_fused = (
            getattr(self.config, "fuse_cross_entropy", True)
            and hidden_kept.is_cuda
            and hidden_kept.numel() > 0
        )
        if use_fused:
            ...
            logits = self.lm_head(hidden_kept)
            per = F.cross_entropy(
            ...
            if p_mask is not None:
                per = per / p_mask.reshape(-1).clamp(min=1e-8).to(per.dtype)
            loss = per.sum()
        if answer_len > 0:
            return loss / answer_len
        # Keep a graph connection so an empty pass-2 batch still backwards.
        if hidden_kept.numel() > 0:
            return hidden_kept.sum() * 0
        return self.lm_head.weight.sum() * 0
    ...
    def forward(
        ...
        relay_h: Optional[torch.Tensor] = None,
        relay_mask: Optional[torch.Tensor] = None,
        relay_layout: Optional[dict] = None,
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if self.training:
            ...
            # ----------------------------------------------------------------
            if relay_layout is not None:
                concat_inputs_ids = relay_layout["concat_input_ids"]
                concat_position_ids = relay_layout["concat_position_ids"]
                flex_attention_mask_3d = relay_layout["attention_mask"]
                logits_to_keep_half = relay_layout["logits_to_keep_half"]
                logits_to_keep = relay_layout["logits_to_keep"]
                p_mask = relay_layout.get("p_mask")
                shifted_labels = relay_layout["shifted_labels"]
                bsz, seq_len = shifted_labels.shape
            else:
                assert input_ids is not None, "input_ids required when relay_layout is not set"
            ...
            # ----------------------------------------------------------------
            embeds = self.model.embed_tokens(concat_inputs_ids)
            embeds = self._inject_relay(embeds, relay_h, relay_mask)
            outputs = self.model(
                input_ids=None,
            ...
            relay_h_last = last_hidden  # (B, 2L, D) last layer only; trainer splits halves
            hidden_states = last_hidden[logits_to_keep].contiguous()
            answer_len = (shifted_labels != -100).sum() if relay_layout is not None else (labels != -100).sum()
            target = shifted_labels[logits_to_keep_half].contiguous()
            if p_mask is None:
                p_mask = torch.ones_like(target, dtype=torch.float, device=target.device)
            loss = self._masked_diffusion_loss(hidden_states, target, p_mask, answer_len)
            logits = self.lm_head(last_hidden) if return_logits else None
        else:
        ...
        if self.training:
            result.relay_h_last = relay_h_last
        return result
